...
class Setting extends BaseModel implements HasMedia
{
    ...
    public static function add($key, $val, $type = 'string', $datatype = null)
    {
        ...
        if($datatype == 'bussiness' || $datatype == 'misc' || $datatype =='notificationconfig'  || $datatype =='appconfig' || $datatype == 'storageconfig'  ){
            ...
            $existingRecord = self::where('name', $key)->where('datatype', $datatype)->first();
    ...
    public static function get($key, $default = null,$datatype = null)
    {
        if (self::has($key)) {
            ...
            if($datatype == 'bussiness' || $datatype == 'misc' || $datatype == 'notificationconfig'|| $datatype =='appconfig'|| $datatype == 'storageconfig'){
                    ...
                    $value = $setting ? self::castValue($setting->val, $setting->type) : self::getDefaultValue($key, $default);
    ...
    public static function getAllSettings($userId = null,$datatype = null)
    {
        if($datatype == 'bussiness' || $datatype == 'misc' || $datatype == 'notificationconfig'|| $datatype =='appconfig'|| $datatype == 'storageconfig'){
            ...
            // For notificationconfig, appconfig, and storageconfig, return settings even when userId is null (for console commands)
            if ($datatype == 'notificationconfig' || $datatype == 'appconfig' || $datatype == 'storageconfig') {
                if ($userId !== null) {
                    $userData = self::where('datatype', $datatype)->where('created_by', $userId)->select('id', 'name', 'val','datatype','created_by')->get();
                   
                    return $userData;
                } else {
                    // Return settings regardless of created_by for system-level settings (console commands)
                    $settings = self::where('datatype', $datatype)->select('id', 'name', 'val','datatype','created_by')->get();
                   
                    return $settings;
                }
            }
            // For bussiness and misc, require userId
            if ($userId !== null) {
