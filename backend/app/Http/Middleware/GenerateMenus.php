    ...
    public function handle()
    {
        ...
        if(isenablemodule('music')==1){
            ...
            $this->childMain($music, [
                'title' => __('sidebar.music_tracks'),
                'route' => 'backend.music.tracks.index',
                'active' => ['app/music/tracks','app/music/tracks/*'],
                'shortTitle' => 'T',
                'order' => 0,
                'permission' => ['view_music_tracks'],
            ...
            $this->childMain($music, [
                'title' => __('sidebar.music_albums'),
                'route' => 'backend.music.albums.index',
                'active' => ['app/music/albums','app/music/albums/*'],
                'shortTitle' => 'A',
                'order' => 0,
                'permission' => ['view_music_albums'],
                'icon' => 'ph ph-album',
            ]);
            
            $this->childMain($music, [
                'title' => __('sidebar.music_playlists'),
                'route' => 'backend.music.playlists.index',
                'active' => ['app/music/playlists','app/music/playlists/*'],
                'shortTitle' => 'P',
                'order' => 0,
...
if($user && $user->hasRole('admin')){
    ...
    $this->mainRoute($menu, [
        ...
        'order' => 10,
            ...
            $user = auth()->user();
            $menu->filter(function ($item) use ($user) {
                if ($item->data('permission')) {
                    if (auth()->check()) {
                        if (\Auth::getDefaultDriver() == 'admin') {
                            return true;
                        }
                        if ($user && $user->hasAnyPermission($item->data('permission'), \Auth::getDefaultDriver())) {
                            return true;
                        }
                    }
